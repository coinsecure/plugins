...
class AccountActionsApi(object):
    ...
    def v1userprofileimagedelete(self, body, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.v1userprofileimagedelete(body, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param NetkiName body: Please enter a valid Netki name associated with your prfile Image. (required)
        :param str authorization: Enter a valid Api Key.
        :param str accept: JSON, XML or CSV can be returned (Optional)
        :return: SuccessResult
                 If the method is called asynchronously,
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method v1userprofileimagedelete" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        # verify the required parameter 'body' is set
        if ('body' not in params) or (params['body'] is None):
            raise ValueError("Missing the required parameter `body` when calling `v1userprofileimagedelete`")


        resource_path = '/v1/user/contact'.replace('{format}', 'json')
        path_params = {}
