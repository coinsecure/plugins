...
class WalletActionsApi(object):
    ...
    def v1userwalletcoinwithdrawunverifiedcancel(self, authorization, body, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.v1userwalletcoinwithdrawunverifiedcancel(authorization, body, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param str authorization: Enter a valid Api Key. (required)
        :param WithdrawID body:  (required)
        :param str accept: JSON, XML or CSV can be returned (Optional)
        :return: SuccessResult
                 If the method is called asynchronously,
                 returns the request thread.
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method v1userwalletcoinwithdrawunverifiedcancel" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        if ('authorization' not in params) or (params['authorization'] is None):
            raise ValueError("Missing the required parameter `authorization` when calling `v1userwalletcoinwithdrawunverifiedcancel`")
        # verify the required parameter 'body' is set
        if ('body' not in params) or (params['body'] is None):
            raise ValueError("Missing the required parameter `body` when calling `v1userwalletcoinwithdrawunverifiedcancel`")


        resource_path = '/v1/user/wallet/coin/withdraw/unverified/cancel'.replace('{format}', 'json')
        path_params = {}
