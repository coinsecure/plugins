...
module SwaggerClient
  class AccountActionsApi
    ...
    # @option opts [String] :accept JSON, XML or CSV can be returned (Optional)
    # @return [SuccessResult]
    def v1userprofileimagedelete(body, opts = {})
      data, _status_code, _headers = v1userprofileimagedelete_with_http_info(body, opts)
      return data
    end
    ...
    # @option opts [String] :accept JSON, XML or CSV can be returned (Optional)
    # @return [Array<(SuccessResult, Fixnum, Hash)>] SuccessResult data, response status code and response headers
    def v1userprofileimagedelete_with_http_info(body, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: AccountActionsApi.v1userprofileimagedelete ..."
      end
      # verify the required parameter 'body' is set
      fail ArgumentError, "Missing the required parameter 'body' when calling AccountActionsApi.v1userprofileimagedelete" if body.nil?
      # resource path
      local_var_path = "/v1/user/contact".sub('{format}','json')
      ...
      data, status_code, headers = @api_client.call_api(:POST, local_var_path,
        ...
        :return_type => 'SuccessResult')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: AccountActionsApi#v1userprofileimagedelete\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
